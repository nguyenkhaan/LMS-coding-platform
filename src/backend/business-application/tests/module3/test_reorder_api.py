...
def setup_teardown():
    ...
    client.post(
        ...
        json={"title": "S1", "position": 1},
    )
    client.post(
        f"/api/teacher/courses/{course_id}/sections",
        json={"title": "S2", "position": 2},
    )
    ...
    client.post(
        f"/api/teacher/courses/2/sections", json={"title": "C2_S1", "position": 1}
    )
...
def test_reorder_course_not_found():
    app.dependency_overrides[get_current_user] = override_get_current_user_teacher
    payload = {
        "items": [{"item_type": "section", "id": 2, "position": 1, "parent_id": None}]
    }
...
def test_reorder_item_not_in_course():
    ...
    payload = {
        "items": [{"item_type": "section", "id": 3, "position": 1, "parent_id": None}]
    }
    response = client.put("/api/teacher/courses/1/curriculum/reorder", json=payload)
    ...
    client.post(
        f"/api/teacher/sections/3/lessons", json={"title": "C2_L1", "position": 1}
    )
    payload = {
        "items": [
            {
                ...
                "position": 1,
                "parent_id": 1,
            }  # Try to move it to section 1 of course 1
        ]
    }
