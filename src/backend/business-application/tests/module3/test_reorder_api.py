...
def setup_teardown():
    ...
    resp = client.post("/api/teacher/courses", json={"title": "Course 1", "description": "", "price": 0, "field": "IT", "tags": []})
    course_id = resp.json()["id"] # Should be 1
    
    # Create section 1 and 2
    client.post(f"/api/teacher/courses/{course_id}/sections", json={"title": "S1", "position": 1})
    client.post(f"/api/teacher/courses/{course_id}/sections", json={"title": "S2", "position": 2})
    
    # Create lessons in section 1
    client.post(f"/api/teacher/sections/1/lessons", json={"title": "L1", "position": 1})
    client.post(f"/api/teacher/sections/1/lessons", json={"title": "L2", "position": 2})

    # Create course 2 (Teacher 1) to test cross-course manipulation
    client.post("/api/teacher/courses", json={"title": "Course 2", "description": "", "price": 0, "field": "IT", "tags": []})
    client.post(f"/api/teacher/courses/2/sections", json={"title": "C2_S1", "position": 1})
...
def test_reorder_success():
    ...
    response = client.put("/api/teacher/courses/1/curriculum/reorder", json=payload)
...
def test_reorder_item_not_in_course():
    ...
    client.post(f"/api/teacher/sections/3/lessons", json={"title": "C2_L1", "position": 1})
    payload = {
        "items": [
