...
def created_course():
    ...
    response = client.post("/api/teacher/courses", json=payload)
...
def test_update_course_success(created_course):
    ...
    response = client.put(f"/api/teacher/courses/{created_course['id']}", json=payload)
    assert response.status_code == 200
    data = response.json()
...
def test_update_course_forbidden():
    ...
    create_resp = client.post("/api/teacher/courses", json=payload)
    ...
    response = client.put(f"/api/teacher/courses/{course_id}", json=payload)
...
def test_get_course_detail_success():
    ...
    response = client.get(f"/api/teacher/courses/{course_id}")
    assert response.status_code == 200
    data = response.json()
...
def test_submit_review_success_from_draft():
    ...
    response = client.post(f"/api/teacher/courses/{course_id}/submit-review")
    assert response.status_code == 200
    data = response.json()
    assert data["status"] == "PENDING_REVIEW"
...
def test_submit_review_invalid_state():
    ...
    client.post(f"/api/teacher/courses/{course_id}/submit-review")
...
def test_reorder_curriculum_success():
    ...
    course_resp = client.post("/api/teacher/courses", json=payload)
    course_id = course_resp.json()["id"]

    # Add 2 sections
    s1_resp = client.post(f"/api/teacher/courses/{course_id}/sections", json={"title": "Sec1", "position": 1})
    s1_id = s1_resp.json()["id"]
    s2_resp = client.post(f"/api/teacher/courses/{course_id}/sections", json={"title": "Sec2", "position": 2})
    ...
    resp = client.put(f"/api/teacher/courses/{course_id}/curriculum/reorder", json=reorder_payload)
...
def test_reorder_curriculum_wrong_course():
    app.dependency_overrides[get_current_user] = override_get_current_user_teacher
    # Create course 1
    c1_resp = client.post("/api/teacher/courses", json={"title": "C1", "description": "D", "price": 10, "thumbnail_url": "u", "field": "IT", "tags": []})
    c1_id = c1_resp.json()["id"]
    s1_resp = client.post(f"/api/teacher/courses/{c1_id}/sections", json={"title": "Sec1", "position": 1})
    s1_id = s1_resp.json()["id"]

    # Create course 2
    c2_resp = client.post("/api/teacher/courses", json={"title": "C2", "description": "D", "price": 10, "thumbnail_url": "u", "field": "IT", "tags": []})
    ...
    resp = client.put(f"/api/teacher/courses/{c2_id}/curriculum/reorder", json=reorder_payload)
    assert resp.status_code == 400
    assert resp.json()["error_code"] == "INVALID_REQUEST"
...
def test_reorder_curriculum_forbidden():
    ...
    resp = client.put(f"/api/teacher/courses/{c1_id}/curriculum/reorder", json=reorder_payload)
...
def test_reorder_curriculum_lesson_content():
    ...
    # Add 1 lesson
    l1_resp = client.post(f"/api/teacher/sections/{s1_id}/lessons", json={"title": "Les1", "summary": "S", "position": 1})
    l1_id = l1_resp.json()["id"]

    # Add 2 contents
    c1_resp = client.post(f"/api/teacher/lessons/{l1_id}/contents", json={"content_type": "READING", "content_id": 1, "media_url": "url", "position": 1})
    c1_id = c1_resp.json()["id"]
    c2_resp = client.post(f"/api/teacher/lessons/{l1_id}/contents", json={"content_type": "READING", "content_id": 2, "media_url": "url", "position": 2})
    c2_id = c2_resp.json()["id"]
...
def test_reorder_curriculum_missing_or_extra_items():
    ...
    resp = client.put(f"/api/teacher/courses/{course_id}/curriculum/reorder", json=reorder_payload_missing)
    assert resp.status_code == 400
    assert resp.json()["error_code"] == "INVALID_REQUEST"
...
def test_delete_lesson_success():
    ...
    c_resp = client.post("/api/teacher/courses", json={"title": "Course for delete", "description": "D", "price": 10, "thumbnail_url": "u", "field": "IT", "tags": []})
    c_id = c_resp.json()["id"]
    s_resp = client.post(f"/api/teacher/courses/{c_id}/sections", json={"title": "Sec", "position": 1})
    s_id = s_resp.json()["id"]
    l_resp = client.post(f"/api/teacher/sections/{s_id}/lessons", json={"title": "Les", "summary": "S", "position": 1})
    l_id = l_resp.json()["id"]

    # Delete lesson
    del_resp = client.delete(f"/api/teacher/lessons/{l_id}")
...
def test_delete_lesson_has_content_blocks():
    ...
    c_resp = client.post("/api/teacher/courses", json={"title": "Course block", "description": "D", "price": 10, "thumbnail_url": "u", "field": "IT", "tags": []})
    c_id = c_resp.json()["id"]
    s_resp = client.post(f"/api/teacher/courses/{c_id}/sections", json={"title": "Sec", "position": 1})
    s_id = s_resp.json()["id"]
    l_resp = client.post(f"/api/teacher/sections/{s_id}/lessons", json={"title": "Les block", "summary": "S", "position": 1})
    l_id = l_resp.json()["id"]
    client.post(f"/api/teacher/lessons/{l_id}/contents", json={"content_type": "READING", "content_id": 1, "position": 1})
...
def test_delete_lesson_wrong_course_state():
    app.dependency_overrides[get_current_user] = override_get_current_user_teacher
    # Create course, section, lesson
    c_resp = client.post("/api/teacher/courses", json={"title": "Course wrong state", "description": "D", "price": 10, "thumbnail_url": "u", "field": "IT", "tags": []})
    c_id = c_resp.json()["id"]
    s_resp = client.post(f"/api/teacher/courses/{c_id}/sections", json={"title": "Sec", "position": 1})
    s_id = s_resp.json()["id"]
    l_resp = client.post(f"/api/teacher/sections/{s_id}/lessons", json={"title": "Les wrong state", "summary": "S", "position": 1})
    l_id = l_resp.json()["id"]

    # Change state to PENDING_REVIEW by submitting
    client.post(f"/api/teacher/courses/{c_id}/submit-review")
...
def test_delete_lesson_forbidden():
    ...
    c_resp = client.post("/api/teacher/courses", json={"title": "C1", "description": "D", "price": 10, "thumbnail_url": "u", "field": "IT", "tags": []})
    c_id = c_resp.json()["id"]
    s_resp = client.post(f"/api/teacher/courses/{c_id}/sections", json={"title": "Sec", "position": 1})
    s_id = s_resp.json()["id"]
    l_resp = client.post(f"/api/teacher/sections/{s_id}/lessons", json={"title": "Les", "summary": "S", "position": 1})
...
def test_create_reading_success():
    app.dependency_overrides[get_current_user] = override_get_current_user_teacher
    # Create course, section, lesson
    c_resp = client.post("/api/teacher/courses", json={"title": "Course read", "description": "D", "price": 10, "thumbnail_url": "u", "field": "IT", "tags": []})
    c_id = c_resp.json()["id"]
    s_resp = client.post(f"/api/teacher/courses/{c_id}/sections", json={"title": "Sec", "position": 1})
    s_id = s_resp.json()["id"]
    l_resp = client.post(f"/api/teacher/sections/{s_id}/lessons", json={"title": "Les", "summary": "S", "position": 1})
    ...
    r_resp = client.post(f"/api/teacher/lessons/{l_id}/readings", json=payload)
...
def test_create_reading_invalid_state():
    app.dependency_overrides[get_current_user] = override_get_current_user_teacher
    c_resp = client.post("/api/teacher/courses", json={"title": "C", "description": "D", "price": 10, "thumbnail_url": "u", "field": "IT", "tags": []})
    c_id = c_resp.json()["id"]
    s_resp = client.post(f"/api/teacher/courses/{c_id}/sections", json={"title": "Sec", "position": 1})
    s_id = s_resp.json()["id"]
    l_resp = client.post(f"/api/teacher/sections/{s_id}/lessons", json={"title": "Les", "summary": "S", "position": 1})
...
def test_update_reading_success():
    app.dependency_overrides[get_current_user] = override_get_current_user_teacher
    c_resp = client.post("/api/teacher/courses", json={"title": "C update", "description": "D", "price": 10, "thumbnail_url": "u", "field": "IT", "tags": []})
    c_id = c_resp.json()["id"]
    s_resp = client.post(f"/api/teacher/courses/{c_id}/sections", json={"title": "Sec", "position": 1})
    s_id = s_resp.json()["id"]
    l_resp = client.post(f"/api/teacher/sections/{s_id}/lessons", json={"title": "Les", "summary": "S", "position": 1})
    ...
    # Create reading
    r_resp = client.post(f"/api/teacher/lessons/{l_id}/readings", json={"title": "R1", "content": "C1", "position": 1})
    lc_id = r_resp.json()["lesson_content"]["id"]

    # Update reading
    update_payload = {"title": "R1 Updated", "content": "C1 Updated"}
    u_resp = client.put(f"/api/teacher/lesson-contents/{lc_id}/reading", json=update_payload)
...
def test_update_reading_invalid_state():
    ...
    u_resp = client.put(f"/api/teacher/lesson-contents/{lc_id}/reading", json={"title": "New"})
...
def test_update_reading_wrong_type():
    ...
    qc_resp = client.post(f"/api/teacher/lessons/{l_id}/contents", json={"content_type": "QUIZ", "content_id": 99, "position": 1})
...
def test_delete_lesson_content_success():
    app.dependency_overrides[get_current_user] = override_get_current_user_teacher
    c_resp = client.post("/api/teacher/courses", json={"title": "C del lc", "description": "D", "price": 10, "thumbnail_url": "u", "field": "IT", "tags": []})
    c_id = c_resp.json()["id"]
    s_resp = client.post(f"/api/teacher/courses/{c_id}/sections", json={"title": "Sec", "position": 1})
    s_id = s_resp.json()["id"]
    l_resp = client.post(f"/api/teacher/sections/{s_id}/lessons", json={"title": "Les", "summary": "S", "position": 1})
