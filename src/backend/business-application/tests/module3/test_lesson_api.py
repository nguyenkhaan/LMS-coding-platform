...
def setup_teardown():
    ...
    # Create section
    resp = client.post(
        f"/api/teacher/courses/{course_id}/sections",
        json={"title": "Section 1", "position": 1},
    )
    yield
...
def test_update_lesson_success():
    app.dependency_overrides[get_current_user] = override_get_current_user_teacher
    resp = client.post(
        "/api/teacher/sections/1/lessons", json={"title": "Old", "position": 1}
    )
...
def test_create_lesson_content_success():
    app.dependency_overrides[get_current_user] = override_get_current_user_teacher
    resp = client.post(
        "/api/teacher/sections/1/lessons", json={"title": "Lesson 1", "position": 1}
    )
    lesson_id = resp.json()["id"]
...
def test_update_lesson_content_success():
    ...
    resp2 = client.post(
        f"/api/teacher/lessons/{lesson_id}/contents",
        json={"content_type": "READING", "content_id": 1, "position": 1},
    )
    content_id = resp2.json()["id"]

    response = client.put(
        f"/api/teacher/lesson-contents/{content_id}", json={"position": 2}
    )
    assert response.status_code == 200
...
def test_cascade_delete_section():
    ...
    # Create lesson
    resp = client.post(
        "/api/teacher/sections/1/lessons", json={"title": "Lesson", "position": 1}
    )
    lesson_id = resp.json()["id"]
    # Create content
    resp2 = client.post(
    ...
    assert (
        client.put(
            f"/api/teacher/lesson-contents/{content_id}",
            json={"content_type": "QUIZ", "content_data": {}},
        ).status_code
        == 404
    )
