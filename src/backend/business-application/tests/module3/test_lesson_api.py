...
def setup_teardown():
    ...
    # Create course
    resp = client.post("/api/teacher/courses", json={"title": "Course", "description": "", "price": 0, "field": "IT", "tags": []})
    course_id = resp.json()["id"]
    # Create section
    resp = client.post(f"/api/teacher/courses/{course_id}/sections", json={"title": "Section 1", "position": 1})
    yield
...
def test_create_lesson_success():
    ...
    response = client.post("/api/teacher/sections/1/lessons", json=payload)
...
def test_update_lesson_success():
    app.dependency_overrides[get_current_user] = override_get_current_user_teacher
    resp = client.post("/api/teacher/sections/1/lessons", json={"title": "Old", "position": 1})
    lesson_id = resp.json()["id"]

    payload = {"title": "New Title", "position": 2}
    response = client.put(f"/api/teacher/lessons/{lesson_id}", json=payload)
    assert response.status_code == 200
    assert response.json()["title"] == "New Title"

def test_create_lesson_content_success():
    app.dependency_overrides[get_current_user] = override_get_current_user_teacher
    resp = client.post("/api/teacher/sections/1/lessons", json={"title": "Lesson 1", "position": 1})
    lesson_id = resp.json()["id"]

    payload = {"content_type": "READING", "content_id": 1, "position": 1}
    response = client.post(f"/api/teacher/lessons/{lesson_id}/contents", json=payload)
    assert response.status_code == 201
...
def test_update_lesson_content_success():
    ...
    resp2 = client.post(f"/api/teacher/lessons/{lesson_id}/contents", json={"content_type": "READING", "content_id": 1, "position": 1})
    content_id = resp2.json()["id"]

    response = client.put(f"/api/teacher/lesson-contents/{content_id}", json={"position": 2})
    assert response.status_code == 200
...
def test_cascade_delete_section():
    app.dependency_overrides[get_current_user] = override_get_current_user_teacher
    # Create lesson
    resp = client.post("/api/teacher/sections/1/lessons", json={"title": "Lesson", "position": 1})
    lesson_id = resp.json()["id"]
    # Create content
    resp2 = client.post(f"/api/teacher/lessons/{lesson_id}/contents", json={"content_type": "READING", "content_id": 1, "position": 1})
    content_id = resp2.json()["id"]
    ...
    # Delete section 1
    resp3 = client.delete("/api/teacher/sections/1")
