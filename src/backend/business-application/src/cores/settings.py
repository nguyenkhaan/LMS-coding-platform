...
def get_env_var(key: str, default: str | _NoArg = NO_ARG) -> str:
    try:
        ...
        if isinstance(default, _NoArg):
            raise ValueError(f"Environment with key {key} is missing") from None
        return default


DATABASE_URL = get_env_var("DATABASE_URL")
VERIFY_REGISTER_SECRET = get_env_var("VERIFY_REGISTER_SECRET")
AUTH_PROVIDER_URL = get_env_var("AUTH_PROVIDER_URL")
...
FRONTEND_CHECKOUT_URL = get_env_var(
    "FRONTEND_CHECKOUT_URL", "http://localhost:5173/checkout"
)
