...
class UserService:
    ...
    async def get_admin_users(self, query: AdminUserListQuery) -> AdminUserListResponse:
        try:
            ...
            if query.role is not None:
                filters.append(UserModel.roles.any(UserRoleModel.role == query.role))
        ...
        try:
            ...
            if previous_roles == requested_roles:
                return UpdateUserRolesResponse(
                    user_id=user.id,
                    roles=[
                        self._to_user_role_view(user_role) for user_role in user.roles
                    ],
            ...
            return UpdateUserRolesResponse(
                user_id=user.id,
                roles=[
                    self._to_user_role_view(user_role) for user_role in new_user_roles
                ],
                capabilities=self._get_capabilities(
    ...
    async def update_teacher_profile(
        ...
        try:
            ...
            stmt = (
                select(TeacherProfileModel)
                .options(selectinload(TeacherProfileModel.registration))
