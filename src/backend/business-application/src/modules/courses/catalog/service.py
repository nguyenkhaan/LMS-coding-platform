...
class CourseDirectoryService:
    ...
    def _eligible_course_statement() -> Select[tuple[CourseModel, float]]:
        ...
        return (
            ...
            .join(
                TeacherProfileModel,
                TeacherProfileModel.user_id == CourseModel.teacher_id,
            )
            .join(
        ...
        row = (
            (
                await self.db.execute(
                    ...
                    .join(
                        TeacherProfileModel, TeacherProfileModel.user_id == UserModel.id
                    )
                    .join(
                        TeacherRegisterModel,
                        TeacherRegisterModel.teacher_profile_id
                        == TeacherProfileModel.user_id,
                    )
                    .join(
                        UserRoleModel,
    ...
    async def _require_enrollment(self, course_id: int, student_id: int) -> None:
        enrollment_id = await self.db.scalar(
    ...
    async def list_courses(
        ...
        if tag:
            normalized_tags = func.regexp_replace(
                func.trim(CourseModel.tags), r"\s*,\s*", ",", "g"
            )
            statement = statement.where(
                func.concat(",", normalized_tags, ",").contains(
                    f",{tag},", autoescape=True
                )
            )
        ...
        total = (
            await self.db.scalar(select(func.count()).select_from(statement.subquery()))
            or 0
        )
        rows = (
            (
                await self.db.execute(
                    statement.order_by(CourseModel.id)
                    .offset((page - 1) * size)
                    .limit(size)
                )
            )
    ...
    async def get_course(
        self, slug: str, current_user_id: int | None
    ) -> CourseDetailView:
        statement = (
        ...
        if current_user_id is not None:
            is_favorited = bool(
                await self.db.scalar(
                    select(CourseFavoriteModel.id).where(
        ...
        review_count = (
            await self.db.scalar(
                select(func.count(CourseReviewModel.id)).where(
    ...
    async def list_instructors(
        ...
        statement = (
            ...
            .join(
                TeacherRegisterModel,
                TeacherRegisterModel.teacher_profile_id == UserModel.id,
            )
            .join(
                UserRoleModel,
                (UserRoleModel.user_id == UserModel.id)
        ...
        if field:
            statement = statement.where(
                TeacherProfileModel.expertise_tags.ilike(f"%{field}%")
            )
        ...
        rows = (
            (
                await self.db.execute(
                    statement.order_by(UserModel.id)
                    .offset((page - 1) * size)
                    .limit(size)
                )
            )
    ...
    async def list_favorites(
        ...
        statement = (
            ...
            .join(
                TeacherRegisterModel,
                TeacherRegisterModel.teacher_profile_id == CourseModel.teacher_id,
            )
            .join(
                UserRoleModel,
        ...
        rows = (
            (
                await self.db.execute(
                    statement.order_by(
                        CourseFavoriteModel.created_at.desc(),
                        CourseFavoriteModel.id.desc(),
                    )
                    .offset((page - 1) * size)
                    .limit(size)
                )
    ...
    async def add_favorite(
        ...
        try:
            ...
            favorite = await self.db.scalar(
                ...
                .options(selectinload(CourseFavoriteModel.course))
                .where(
                    CourseFavoriteModel.student_id == student_id,
    ...
    async def remove_favorite(
        ...
        await self._lock_student(student_id)
        _ = await self.db.execute(
            delete(CourseFavoriteModel).where(
                CourseFavoriteModel.student_id == student_id,
                CourseFavoriteModel.course_id == course_id,
            )
        )
    ...
    async def list_reviews(
        ...
        statement = select(CourseReviewModel).where(
        ...
        reviews = (
            await self.db.scalars(
                statement.order_by(
                    CourseReviewModel.created_at.desc(), CourseReviewModel.id.desc()
                )
                .offset((page - 1) * size)
                .limit(size)
            )
    ...
    async def add_review(
        ...
        if (
            await self.db.scalar(
                select(CourseReviewModel.id).where(
                    CourseReviewModel.course_id == course_id,
