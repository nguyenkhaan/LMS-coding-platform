...
class StudentService:
    ...
    async def list_progress(
        ...
        progresses = list(
            (
                await self.db_session.scalars(
                    statement.order_by(LessonContentProgressModel.id)
                    .offset((page - 1) * size)
                    .limit(size)
                )
            ).all()
    ...
    def _progress_statement(
        ...
        statement = (
            ...
            .join(
                LessonContentModel,
                LessonContentModel.id == LessonContentProgressModel.lesson_content_id,
            )
    ...
    async def _records_by_id(
        ...
        if not ids:
            return {}
        records = (
            await self.db_session.scalars(select(model).where(id_column.in_(ids)))
        ).all()
    ...
    def _content_view(
        ...
        if expected_record is None:
            raise HTTPException(
                status_code=409, detail="Invalid lesson content binding"
            )
