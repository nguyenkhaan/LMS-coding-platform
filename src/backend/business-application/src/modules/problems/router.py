...
from src.modules.problems.dto import (
    ProblemTagView,
    ProblemView,
    ProblemWrite,
...
from src.modules.problems.upload_validation import validate_testcase_file
...
@teacher_problem_router.post(
    "/problems", response_model=ProblemView, status_code=status.HTTP_201_CREATED
)
...
@teacher_problem_router.post(
    "/problems/{problem_id}/testcases/upload",
    response_model=TestcaseUploadResponse,
...
async def upload_testcase(
    ...
    if not input_file or not output_file:
        raise HTTPException(
            status_code=400, detail="Both input_file and output_file are required"
        )

    validate_testcase_file(input_file)
    validate_testcase_file(output_file)
