...
class TeacherProblemService:
    ...
    async def update_problem(
        self, teacher_id: int, problem_id: int, data: ProblemWrite
    ) -> ProblemView:
        ...
        if problem.teacher_id != teacher_id:
            raise HTTPException(
                status_code=403,
                detail="You do not have permission to modify this problem",
            )
        ...
        stmt_del_tags = delete(ProblemTagMappingModel).where(
            ProblemTagMappingModel.problem_id == problem.id
        )
        ...
        stmt_get_configs = select(ProblemConfigModel).where(
