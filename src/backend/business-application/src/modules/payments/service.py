...
class PaymentService:
    ...
    async def create_payment(
        self, student_id: int, course_id: int, idempotency_key: str | None
    ) -> PaymentTransactionView:
        ...
        if not course or course.deleted_at is not None:
            raise HTTPException(
                status_code=404,
                detail={
                    "message": "Không tìm thấy khóa học hoặc đã bị xóa",
        ...
        if course.status not in (CourseStatus.APPROVED, "PUBLISHED"):
            raise HTTPException(
                status_code=400,
                detail={
                    "message": f"Trạng thái khóa học {course.status} không cho phép đăng ký",
        ...
        if course.price <= 0:
            raise HTTPException(
                status_code=400,
                detail={
                    "message": "Khóa học này miễn phí, vui lòng sử dụng API đăng ký miễn phí",
        ...
        scoped_idemp_key = (
            f"{student_id}:{idempotency_key}" if idempotency_key else str(uuid.uuid4())
        )
        if idempotency_key:
        ...
        try:
            ...
            raise HTTPException(
                status_code=502, detail=f"PayOS is unavailable: {e}"
            ) from e
    ...
    async def process_webhook(self, payload: PayOSWebhookRequest) -> dict:
        ...
        if not is_valid:
            logger.warning(
                "PayOS Webhook Signature không hợp lệ: %s", payload.signature
            )
            raise HTTPException(
                status_code=400,
                detail={
        ...
        if course:
            ...
            wallet.available_balance = round(
                float(wallet.available_balance) + float(tx.amount), 2
            )
        ...
        return {
            "status": "ok",
            "message": "Xử lý webhook và kích hoạt khóa học thành công",
        }

    async def get_transaction_status(
        self, transaction_code: str, user_id: int, is_admin: bool = False
    ) -> TransactionStatusResponse:
        """Lấy trạng thái giao dịch phục vụ polling từ phía frontend."""
        filters = [TransactionModel.transaction_code == transaction_code]
        if not is_admin:
            filters.append(TransactionModel.student_id == user_id)
    ...
    async def cancel_transaction(
        ...
        if tx.status == PaymentStatus.COMPLETED:
            raise HTTPException(
                status_code=400,
                detail={
                    "message": "Không thể hủy giao dịch đã hoàn tất",
                    "error_code": "INVALID_STATE",
                },
            )
    ...
    async def get_admin_payments(
        self, page: int, size: int, status: PaymentStatus | None = None
    ) -> PaginatedPaymentTransactionView:
        ...
        for item in items:
            order_code = (
                int(item.transaction_code.replace("TXN-", ""))
                if item.transaction_code.startswith("TXN-")
                else 0
            )
            data.append(
                PaymentTransactionView(
        ...
        return PaginatedPaymentTransactionView(
            data=data, pagination={"page": page, "size": size, "total": total}
        )
