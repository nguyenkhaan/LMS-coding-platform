...
class PaymentTransactionView(BaseModel):
    transaction_code: str = Field(..., description="Mã giao dịch hệ thống LMS")
    order_code: int = Field(..., description="Mã đơn hàng PayOS")
    payos_link: str = Field(
        ..., description="Link thanh toán PayOS", alias="checkout_url"
    )
    qrcode: str | None = Field(
        default=None, description="Chuỗi VietQR raw để client tự render QR code nếu cần"
    )
    amount: float = Field(description="Số tiền thanh toán (USD)")
    currency: str = Field(default="USD", description="Đơn vị tiền tệ chuẩn (USD)")
    status: PaymentStatus = Field(
        default=PaymentStatus.PENDING, description="Trạng thái giao dịch"
    )
    expires_at: datetime | None = Field(
        default=None, description="Thời điểm hết hạn của link thanh toán"
    )
...
class PayOSWebhookRequest(BaseModel):
    ...
    data: PayOSWebhookData = Field(
        ..., description="Dữ liệu chi tiết giao dịch ngân hàng"
    )
