...
class TeacherApplicationService:
    ...
    def _history_view(
        self, history: TeacherRegisterHistoryModel
    ) -> TeacherApplicationHistoryView:
    ...
    async def _application_view(
        ...
        *,
        ...
        if include_sensitive and minio_handler is None:
            raise RuntimeError(
                "A MinIO handler is required for sensitive application views"
            )
    ...
    async def _me_response(
        ...
        return TeacherApplicationMeResponse(
            ...
            history=[
                self._history_view(item)
                for item in sorted(app.history, key=lambda item: item.id)
            ],
            can_edit=app.status
            in (
    ...
    async def create_application(
        ...
        try:
            ...
            uploaded = await self._upload_documents(
                documents, minio_handler, new_app.id
            )
