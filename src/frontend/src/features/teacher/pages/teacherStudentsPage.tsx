        ...
        <div className="flex-1 w-full flex flex-col gap-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="relative w-full sm:max-w-xs">
              ...
              <input
                ...
                className="w-full pl-9 pr-3.5 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#392C7D] transition-all bg-white text-[#111827]"
              />
            </div>
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <div className="flex items-center gap-2 w-full sm:w-auto">
                ...
                <select
                  ...
                  className="px-3.5 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#392C7D] transition-all bg-white text-[#374151] w-full sm:w-auto"
                >
              ...
              <button
                ...
                title="Refresh list"
              >
                <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin text-[#392C7D]' : ''}`} />
              </button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm flex flex-col gap-6">
            <div className="flex items-center justify-between border-b border-gray-150 pb-4">
              <h3 className="text-base font-bold text-[#111827]">Active Enrolled Students</h3>
            ...
            {loading ? (
              <div className="py-20 flex flex-col items-center justify-center gap-3">
                <RefreshCw className="w-8 h-8 text-[#392C7D] animate-spin" />
              ...
              <div className="py-20 flex flex-col items-center justify-center gap-3 text-center">
                ...
                <h4 className="text-base font-bold text-[#111827]">No Students Found</h4>
                  ...
                  >
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      <div className="flex items-center gap-3">
                        <div className="w-11 h-11 rounded-xl bg-indigo-50 border border-indigo-100 flex items-center justify-center text-[#392C7D] font-extrabold text-sm shrink-0">
                          {student.name.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase()}
                        </div>
                        <div className="flex flex-col">
                          <span className="text-sm font-extrabold text-[#111827]">{student.name}</span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-slate-50 pt-4 mt-1">
                      <div className="flex flex-col gap-2">
                        <div className="flex justify-between items-center text-xs font-semibold">
                          <span className="text-[#374151] truncate max-w-[200px] sm:max-w-xs">{student.course}</span>
                          <span className="text-indigo-950 font-bold">{student.progress}%</span>
                        </div>
                        <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                          <div 
                            className="bg-[#392C7D] h-full rounded-full transition-all duration-500" 
