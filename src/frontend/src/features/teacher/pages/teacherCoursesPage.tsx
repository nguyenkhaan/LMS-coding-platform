      ...
      <div className="w-full max-w-[1340px] mx-auto px-4 py-10 flex flex-col lg:flex-row gap-8 items-start">
        ...
        <div className="flex-1 w-full flex flex-col gap-6 min-w-0">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
            <div>
              <h3 className="text-[20px] font-bold text-zinc-900">My Courses</h3>
              <p className="text-[13px] text-neutral-500 mt-0.5">Create, configure, and publish your course curriculum.</p>
            </div>
            <button
              onClick={handleCreateCourse}
              className="flex items-center gap-2 px-5 py-2.5 bg-primary text-white text-sm font-bold rounded-xl hover:bg-primary-hover transition-all cursor-pointer shadow-sm"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div key={course.id} className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow">
                  ...
                  <div className="p-5 flex-1 flex flex-col gap-3">
                    <div className="flex justify-between items-start gap-2">
                      <span className="text-[11px] font-bold text-accent bg-rose-50 px-2 py-0.5 rounded-full tracking-wide">
                        {course.field}
                      </span>
                    ...
                    <div>
                      <h4 className="text-[16px] font-semibold text-zinc-900 line-clamp-1 hover:text-primary transition-colors">
                        {course.title}
                      </h4>
                      <p className="text-[13px] text-zinc-700 mt-1 line-clamp-2">
                        {course.description}
                      </p>
                    </div>

                    {/* Stats List */}
                    <div className="flex flex-wrap items-center gap-4 text-xs font-semibold text-neutral-500 border-t border-b border-gray-100 py-3 mt-2">
                    ...
                    <div className="flex flex-col gap-1 text-[11px] text-neutral-500 font-medium pt-1">
                  ...
                  <div className="bg-slate-50 px-5 py-4 border-t border-gray-100 flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 rounded-xl border border-gray-200 text-xs font-bold text-zinc-700 hover:bg-white transition-all cursor-pointer flex items-center gap-1"
                      >
                      ...
                      {(isPublished || isPending) ? (
                        <button
                          ...
                          className="px-3.5 py-1.5 rounded-xl bg-indigo-50 border border-indigo-100 text-xs font-bold text-primary hover:bg-indigo-100 transition-all cursor-pointer flex items-center gap-1"
                        >
                        ...
                        <button
                          onClick={() => navigate(`/teacher/courses/${course.id}/edit`)}
                          className="px-3.5 py-1.5 rounded-xl bg-accent text-white text-xs font-bold hover:bg-accent-hover transition-all cursor-pointer flex items-center gap-1"
                        >
            ...
            {filteredCourses.length === 0 && (
              <div className="col-span-1 md:col-span-2 text-center py-16 bg-white rounded-2xl border border-gray-200 shadow-sm flex flex-col justify-center items-center gap-3">
                ...
                <h4 className="text-[16px] font-bold text-zinc-900">No Courses Found</h4>
                <p className="text-[13px] text-neutral-500">Create a new course or change your tab filter selection.</p>
              </div>
            )}
          </div>
